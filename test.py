...
def run_an_eval_epoch(smiles_list,args, model, data_loader):
    ...
    with torch.no_grad():
        ...
        # Load existing CSV file and add predictions column
        out=pd.read_csv(args['test_csv_path'],header=None, names=['smiles'])
...
if __name__ == '__main__':
    ...
    args['test_csv_path'] = "example.csv"

    # csv path to put the prediction 
    args['predict_results_csv_path'] = "results/predict.csv"

    # Initialize and set up experiment arguments
    args = setup(args)

    # Load the test set from a CSV file and add placeholder columns
    test_set = pd.read_csv(args['test_csv_path'],header=None, names=['smiles'])
